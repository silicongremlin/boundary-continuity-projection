...
# ============================================================
# SIM_PROJECT_4D_CONTROL_SQUARE_BOUNDARY
#
# Control test:
#   Replace circular/ring boundary with square boundary.
#
# Keeps:
...
#
# Changes:
#   - boundary B(x,y) is square / box-shell topology
#
# Question:
#   Does boundary shape/topology change the ejection/admission behavior?
# ============================================================
...
# square boundary shell
square_half_size = 1.25
sigma = 0.22
...
# 2D VIEW
# -----------------------------
ax2d.set_title("CONTROL: Square Boundary")
ax2d.set_xlim(-3, 3)
ax2d.set_ylim(-3, 3)
...
# Square boundary visual
s = square_half_size
outer = s + sigma
inner = s - sigma

for side in [s, outer, inner]:
    alpha = 1.0 if side == s else 0.45
    linestyle = "-" if side == s else "--"
    ax2d.plot([-side, side, side, -side, -side], [-side, -side, side, side, -side],
              linewidth=1.2 if side == s else 0.6, linestyle=linestyle, alpha=alpha)

# Boundary field background
grid_n = 180
gx: FloatArray = np.linspace(-3, 3, grid_n, dtype=np.float64)
gy: FloatArray = np.linspace(-3, 3, grid_n, dtype=np.float64)
GX, GY = np.meshgrid(gx, gy)

def square_boundary_strength_grid(x: FloatArray, y: FloatArray) -> FloatArray:
    # Chebyshev radius: square shells are level sets of max(|x|, |y|)
    q = np.maximum(np.abs(x), np.abs(y))
    return np.exp(-((q - square_half_size) ** 2) / (2 * sigma**2)).astype(np.float64)

BG = square_boundary_strength_grid(GX, GY)
...
# 3D VIEW
# -----------------------------
ax3d.set_title("3D Projection: Square Boundary")
ax3d.set_xlim(-3, 3)
ax3d.set_ylim(-3, 3)
ax3d.set_zlim(-3, 3)
ax3d.set_xlabel("x′")
...
# Square boundary visual in 3D at z=0
ax3d.plot([-s, s, s, -s, -s], [-s, -s, s, s, -s], np.zeros(5), linewidth=1.0)
...
# METRICS
# -----------------------------
axm.set_title("CONTROL Metrics: Square Boundary")
axm.set_xlim(0, 900)
axm.set_ylim(0, 2.25)
...
line_trap, = axm.plot([], [], label="edge/trap fraction")
...
def boundary_gradient(x: FloatArray, y: FloatArray) -> tuple[FloatArray, FloatArray]:
    """
    Gradient of square-shell boundary field.
    Defines local normal for square boundary.
    """
    eps = 1e-3
...
def square_phi(x: FloatArray, y: FloatArray) -> FloatArray:
    """
    Angular-ish coordinate retained for phase modulation.
    For square boundary, still use atan2 so comparison stays close
    to ring/random controls.
    """
...
def latent_vector_field_square_boundary(
    s: FloatArray,
    frame: int,
    ...
    # Square-boundary local normal from gradient
    gn_x, gn_y = boundary_gradient(x, y)

    # Tangent along square boundary
    gt_x = -gn_y
    gt_y = gn_x
...
def update(frame: int) -> tuple[Any, ...]:
    ...
    field_vel, B = latent_vector_field_square_boundary(state4, frame)
    ...
    info.set_text(
        ...
        f"edge/trap fraction = {trap:.3f}\n"
        f"CONTROL = SQUARE BOUNDARY"
    )
